...
/* none_proc1 is ()->int; these slots in fbdemo wipe-proc arrays are called
 * through typed function pointers. wasm call_indirect strictly type-checks. */
static void pc_fbdemo_cleanup_none(fbdemo_wipe_u* u) { (void)u; }
#endif
...
#ifdef TARGET_PC
#define FBDEMO_NO_CLEANUP pc_fbdemo_cleanup_none
#else
...
static int _none_DRAW_CHK_PROC(ACTOR* _p0, GAME_PLAY* _p1) { (void)_p0; (void)_p1; return 0; }
static void _none_PLAY_WIPE_PROC(GAME_PLAY* _p0) { (void)_p0; }
#endif

static void Game_play_fbdemo_wipe_proc(GAME_PLAY* play) {
    static PLAY_WIPE_PROC wipe_proc[] = { _none_PLAY_WIPE_PROC, Game_play_fbdemo_wipe_create,
...
extern void play_init(GAME* game) {
    ...
    play->draw_chk_proc = _none_DRAW_CHK_PROC;
    mMsg_ct(game);
