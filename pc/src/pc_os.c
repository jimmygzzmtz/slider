 ...
 * --wrap=printf/iprintf/puts in pc/CMakeLists.txt routes those calls
 * here; in normal runs the flag is off and the wraps short-circuit
 * before crossing the wasm→JS bridge (~100–500 µs per crossing;
 * jaudio fires several per voice setup, producing a ~25 ms press-time
 * hitch). With --verbose the originals run and [TRG_ALLOC]/[TRG_P5]/
 * etc. reach the console.
 *
 * Only the bare printf-family calls clang -O2 rewrites stdout output
 * into are gated: printf, puts, iprintf. fprintf/fputs/vfprintf go
 * through unwrapped — they're rare (engine errors, FATAL messages),
 * always need to be visible, and forwarding va_list across a wasm32
 * wrap was dropping arg slots in practice (empty %s in fprintf). */
extern int __real_puts(const char* s);

/* Forward via vsnprintf+__real_puts rather than __real_vprintf so we
 * don't need --wrap=vprintf (which would require us to also provide
 * __wrap_vprintf and stay coherent with the family). 4 KB buffer is
 * enough for any single decomp/jaudio log line; longer ones get
 * truncated, which is fine for diagnostics. */
int __wrap_printf(const char* fmt, ...) {
    ...
    va_end(ap);
    /* strip trailing newline — __real_puts adds one of its own */
    if (n > 0 && buf[n - 1] == '\n') buf[n - 1] = '\0';
    return __real_puts(buf);
}
