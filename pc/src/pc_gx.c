...
void GXSetTevColor(GXTevRegID id, GXColor color) {
    ...
    if (id < GX_MAX_TEVREG) {
        ...
        if (id == GX_TEVREG0) {
            ...
            /* TEVREG1/2/3 are emu64 EmuColor values packed as (R<<24|G<<16|B<<8|A)
             * and reinterpreted via GXColor on little-endian hosts. */
            c[0] = color.a / 255.0f;
            c[1] = color.b / 255.0f;
            c[2] = color.g / 255.0f;
            c[3] = color.r / 255.0f;
...
void GXSetTevColorS10(GXTevRegID id, GXColorS10 color) {
    ...
    if ((u32)id < GX_MAX_TEVREG) {
        ...
        if (memcmp(g_gx.tev_colors[id], c, sizeof(c)) == 0) {
            return;
        }
        DIRTY(PC_GX_DIRTY_TEV_COLORS);
        memcpy(g_gx.tev_colors[id], c, sizeof(c));
...
void GXLoadLightObjImm(void* lt, u32 light) {
    ...
    /* l->color is packed as (R<<24|G<<16|B<<8|A) per the GX SDK convention. */
